...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) < prev and len(grid.get_cells()[i][j]) > 1:
                    prev = len(grid.get_cells()[i][j])
                    tup = i, j
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while len(Q) > 0:
            row, col = Q.pop()
            var_assigned_row, fail_row = self.remove_domain_row(grid, row, col)
            var_assigned_col, fail_col = self.remove_domain_column(grid, row, col)
            var_assigned_unit, fail_unit = self.remove_domain_unit(grid, row, col)

            if fail_row or fail_col or fail_unit:
                return False
            Q.update(var_assigned_row, var_assigned_col, var_assigned_unit)
    ...
    def search(self, grid, var_selector):
        ...
        for d in grid.get_cells()[i][j]:
            if grid.is_value_consistent(d, i, j):
                copy = grid.copy()
                copy.get_cells()[i][j] = d
                check = AC3().consistency(copy, {(i, j)})
                if check:
...
for p in problems:
    ...
    # Call search with the first available heuristic
    print("Solution with first available heuristic")
    FAstart = time.time()
    solution = bt.search(g, FirstAvailable())
    solution.print()
    FAend = time.time()
    print("Is the current grid a solution? ", solution.is_solved())
    print('\n')

    # Call search with the MRV heuristic
    print("Solution with MRV heuristic")
    MRVstart = time.time()
    solution2 = bt.search(g, MRV())
    solution2.print()
    MRVend = time.time()
    print("Is the current grid a solution? ", solution2.is_solved())

    running_time_first_available.append(FAend - FAstart)
    running_time_MRV.append(MRVend - MRVstart)
